...
class UserTest < ActiveSupport::TestCase
  ...
  test "email_validation" do 
    ...
    max_len = 255
    domain = "@example.com"
    email = "a" * (max_len + 1 - domain.length) + domain
    user.email = email
    user.save
    maxlength_msg = ["メールアドレスは#{max_len}文字以内で入力してください"]
    assert_equal(maxlength_msg, user.errors.full_messages)
  ...
  test "active_user_uniqueness" do
    ...
    # アクティブユーザー（activated: true）がいない場合
    # 同じemailが保存できること
    count = 3
    ...
    # アクティブユーザー（activated: true）がいる場合
    # 同じemailが保存できないこと
    ...
    # アクティブユーザー（activated: true）がいなくなった場合
    # 同じemailが保存できること
    active_user.destroy!
    assert_difference("User.count", 1) do
